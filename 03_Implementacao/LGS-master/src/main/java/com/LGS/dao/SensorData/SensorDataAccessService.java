...
public class SensorDataAccessService implements SensorDataDao {
    ...
    @Override
    public int insertDataProcessed(UUID id, SensorDataBundle data, int cautionLevel, String cautionDescription) {
        try {
            StringBuilder query = new StringBuilder("INSERT INTO SENSORDATA (pId,uId,sId,sValue,sTimestamp," +
                    "sCaution,sCautionDescription,sProcessedDelayedHigh,sProcessedDelayedMedium,sProcessedDelayedLow) VALUES ");
            for (SensorPacket packet : data.getSensorData()) {
                query.append(String.format("('%s','%s',(SELECT sId FROM SENSORS WHERE sName= '%s'), %f, " +
                                "'%s', '%d','%s', '%s', '%s', '%s'),",
                        id, data.getUserId(), packet.getSensorType(), packet.getSensorReading(),
                        new Timestamp(System.currentTimeMillis()), cautionLevel, cautionDescription, '0', '0', '0'));
            }
            query = new StringBuilder(query.substring(0, query.length() - 1));
            jdbcTemplate.update(query.toString());
            return 1;
    ...
    @Override
    public Optional<SensorDataBundleView> selectUnprocessedSensorDataByUserId(UUID userId, String cautionMeasurement) {
        try {
            List<SensorPacket> SensorPackets = getUserUnprocessedPackets(userId, cautionMeasurement);
            SensorDataBundleView sensorData = new SensorDataBundleView(SensorPackets);
            return Optional.of(sensorData);
        } catch (EmptyResultDataAccessException e) {
            return Optional.empty();
    ...
    @Override
    public int updateSensorDataByUserId(UUID userID, SensorDataBundleView packetsToUpdate, String cautionMeasurement) {
        try {
            for (SensorPacket packet : packetsToUpdate.getSensorData()) {
                int value = updateSensorDataByUserId(userID, packet, cautionMeasurement);
                if (value == 0) {
    ...
    private List<SensorPacket> getUserSensorPackets(UUID userId) {
        ...
        return jdbcTemplate.query(query, new Object[]{userId}, (resultSet, i) -> {
            ...
            float value = resultSet.getFloat("sValue");
            Timestamp timestamp = resultSet.getTimestamp("sTimestamp");
            int caution = resultSet.getInt("sCaution");
            String description = resultSet.getString("sCautionDescription");
            return new SensorPacket(sType, String.valueOf(value), timestamp, caution, description);
        });
    }


    private List<SensorPacket> getUserUnprocessedPackets(UUID userId, String cautionMeasurement) {
        String query = "SELECT * FROM SENSORDATA AS SD JOIN SENSORS AS S ON SD.sId = S.sId WHERE uId= '" + userId + "' AND " + cautionMeasurement + " = 'f' ORDER BY stimestamp";
        return jdbcTemplate.query(query, (resultSet, i) -> {
            String sType = resultSet.getString("sName");
            float value = resultSet.getFloat("sValue");
            Timestamp timestamp = resultSet.getTimestamp("sTimestamp");
            int cautionLevel = resultSet.getInt("sCaution");
            String cautionDescription = resultSet.getString("sCautionDescription");
            return new SensorPacket(sType, String.valueOf(value), timestamp, cautionLevel, cautionDescription);
        });
    }
    ...
    /**
     * Update flag for packet processed for high caution measurement
     *
     * @param userId user id
     * @param packet packet
     * @return 1 if updated 0 if not
     */
    private int updateSensorDataByUserId(UUID userId, SensorPacket packet, String cautionMeasurement) {
        String sql = "" +
                "UPDATE SENSORDATA " +
                "SET " + cautionMeasurement + " = '1'" +
                "WHERE uId= '" + userId + "' AND sId =(SELECT sId FROM SENSORS WHERE sName='" + packet.getSensorType() + "') AND sValue = '" + packet.getSensorReading() +
                "' AND sTimestamp = '" + packet.getTimestamp() + "' AND sCaution = " + packet.getCautionLevel() +
                " AND sCautionDescription = '" + packet.getCautionDescription() + "'";
        return jdbcTemplate.update(sql);
    }
